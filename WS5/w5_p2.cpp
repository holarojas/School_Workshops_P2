...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		if (!fileBooks) {
			...
			while (library.size() < TOP_BOOKS) { //Load only first 4 books
				getline(fileBooks, strRecord);

				if (!fileBooks.eof()) {
					if (strRecord[0] != '#') {
						library += sdds::Book(strRecord);
					}
				}
			...
			//library.setObserver(nullptr);


			// TODO: add the rest of the books from the file.

			while (!fileBooks.eof()) { //Load the rest of the books
				getline(fileBooks, strRecord);
	...
	if (argc > 2) {
		...
		if (!fileMovies) {
			...
			for (auto i = 0u; i < TOP_MOVIES; ++i) { //Load only first 5 movies
				getline(fileMovies, strRecord);

				if (!fileMovies.eof()) {
					if (strRecord[0] != '#') {
						movies[i] = sdds::Movie(strRecord);
					}
					else {
						i = -1;
					}
	...
	for (auto i = 3; i < argc; ++i)
	{
		...
		catch (const std::out_of_range& err) //Throwing from Operator[] overload
		{
			std::cerr << "** EXCEPTION: " << err.what() << std::endl;;
		}
		catch (const char* err) //Throwing from SpellChecker custom constructor
		{
